...
try:
    ...
    print("Models not available, running in basic mode")
...
# Database setup (only if models are available)
if MODELS_AVAILABLE:
    try:
...
def chat():
    try:
        ...
        # Get client medical context (simplified version without RAG)
        client_context = ""
        if MODELS_AVAILABLE and db_session:
            try:
                client = db_session.query(Client).filter_by(id=client_id).first()
                if client:
        ...
        response, history = send_message_with_context(user_message, history, context, "", client_context)
...
def health_check():
    ...
    return jsonify({
        "status": "healthy",
        "database_available": MODELS_AVAILABLE,
        "version": "simple"
    }), 200
